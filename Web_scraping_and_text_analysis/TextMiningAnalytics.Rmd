...
author: "Granger Huntress/Vikrant Verma"
date: "3/5/2019"
...
# Let us clear the workspace and load the packages
rm(list=ls())
...
```
Why is this a little different than the previous frequency matrix? How can cat jump from 19 to 22 instances?
...
This reads like 84682 cells in frequencies are 0, 17471 have non-zero values. 83% of all cells are zero (which is 84682/(84682+17471))
...
# barplot(term_frequency[1:15],col = "tan", las=1)
barplot(term_frequency[1:15],col = "tan", las=2)
# barplot(term_frequency[1:15],col = "tan", las=3)
...
new_tdm<-removeSparseTerms(ps_TDM, sparse = 0.94)
new_tdm

```

removeSparseTerms(ps_TDM, sparse = 0.94) removes those terms in frequencies, for which at least 94% of all cells are zero, i.e. which are quite uncommon in the corpus. As a result, you get a new TermDocumentMatrix with 2488 terms and only 28346 zero entries
...
new_tdm<-removeSparseTerms(tdm, sparse = 0.94)
