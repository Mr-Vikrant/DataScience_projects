...
Let's load the books - Jane Austen's Complete Novels.
```{r}

titles <- c("Sense and Sensibility","Pride and Prejudice","Mansfield Park","Emma","Northanger Abbey","Persuasion")
#titles
books <- list(sensesensibility,prideprejudice,mansfieldpark,emma,northangerabbey,persuasion) #library(janeaustenr)
books
##Each book is an array in which each value in the array is a chapter 
series <- tibble() #library(dplyr)
series
for(i in seq_along(titles)) {

  temp <- tibble(chapter = seq_along(books[[i]]),
  text = books[[i]]) %>%
  unnest_tokens(word, text) %>% #library(tidytext)
...
series %>% 
  ...
  dplyr::count(word) %>% 
  with(wordcloud(word, n, max.words = 50)) #library(wordcloud)
...
series %>%
  ...
  acast(word ~ sentiment, value.var = "n", fill = 0) %>% #library(reshape2)
...
bigrams_separated <- series %>%
  separate(bigram, c("word1", "word2"), sep = " ") #library(tidyverse) 
...
bigram_graph <- bigrams_counts %>%
  filter(n > 70) %>%
  graph_from_data_frame() #library(igraph)
bigram_graph
...
set.seed(2019)
ggraph(bigram_graph, layout = "fr") + #library(ggraph)
  geom_edge_link() +
  geom_node_point() +
